...
function Home() {
  const { getAllProduct } = useProductFunction();
  ...
  return (
    <div className="container d-flex justify-content-center flex-wrap gap-4 bg-body-tertiary p-4 shadow rounded" style={{ marginTop: "50px" }}>
      {productData.map((item, index) => (
        <div key={index} className="card " style={{ width: "18rem" }}>
          ...
          <div className="card-body ">
            <Link to={`/product/${item.id}`} className="card-title h5 fw-bold">
              {item.productName}
            </Link>
            <p className="card-text fw-bold" style={{ color: item.category === "Weapons" ? "red" : item.category === "Armor" ? "gold" : "black" }}>
              {item.category}
            </p>
            <p className="card-text">{item.description.length > 100 ? `${item.description.substring(0, 100)}...` : item.description}</p>
          </div>
          <p className="card-text text-success p-3">{item.price ? Number(item.price).toLocaleString("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0, maximumFractionDigits: 0 }) : ""}</p>
        </div>
      ))}
    </div>
  );
